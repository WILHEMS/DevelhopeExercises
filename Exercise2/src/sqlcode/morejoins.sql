...
select NAME, PRICE, QUANTITY from INVENTORY left join product on PRODUCT.PRODUCT_ID=INVENTORY.PRODUCT_ID where QUANTITY<200;

/* right join with where*/
select NAME, PRICE, QUANTITY from INVENTORY inner join product on PRODUCT.PRODUCT_ID=INVENTORY.PRODUCT_ID where PRICE>400;