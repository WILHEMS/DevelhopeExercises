...
select format(sum(meal_price), 2) from meal_orders where meal_price>15;
...
select sum(meal_price) from meals where name like '%S%';
...
select meal, count(meal) from meal_orders where date_of_order<'2023-09-08 08:00:00' group by meal;